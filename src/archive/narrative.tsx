...
export const narrativeEssay = new BaseEssayQuestion(
  ...
  async (prisma: PrismaClient, criteria, answer: EssayAnswer, data: string) => {
    ...
    const contentLevel = Number(answerResponse.Content?.level ?? "0");
    const contentMarks =
      (contentLevel / 6) *
      (criteria.find((criterion) => criterion.name == "Content")?.marks ?? 5);

    const structureLevel = Number(answerResponse.Structure?.level ?? "0");
    const structureMarks =
      (structureLevel / 6) *
      (criteria.find((criterion) => criterion.name == "Structure")?.marks ?? 5);
    ...
    const criteriaMarks = {
      Content: contentMarks,
      Structure: structureMarks,
