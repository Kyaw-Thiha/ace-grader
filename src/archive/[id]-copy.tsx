...
export function getServerSideProps(context: GetServerSidePropsContext) {
  ...
  const {
    data: questionWorksheet,
    refetch: refetchQuestions,
    isLoading,
    isError,
  } = api.worksheet.getQuestions.useQuery({ id: worksheetId });
  const questions = questionWorksheet?.questions;
      ...
      >
        <div className="flex min-h-[80vh] flex-col items-center justify-center gap-16">
          ...
          <div className="flex items-center justify-center">
            <AddQuestionButton id={id} order={1} refetch={refetchWorksheet} />
          </div>
        </div>
      </WorksheetLayout>
    );
...
type Worksheet = RouterOutputs["worksheet"]["get"];
...
const WorksheetLayout: React.FC<WorksheetHeaderProps> = ({
  ...
  const updateTitle = () => {
    if (title != "") {
      editTitle.mutate({ id: id, title: title });
    }
  };
      ...
      <div className="flex min-h-screen flex-col">
        <header className="border-b">
          <nav className="container mx-auto flex justify-between px-4 py-4">
            ...
            <PublishWorksheetButton worksheetId={id} refetch={refetch} />
          </nav>
        </header>
